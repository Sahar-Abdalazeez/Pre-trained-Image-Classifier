...
#                                                                             
# PROGRAMMER:  Sahar Samer Abd Al-Aziz
...
def calculates_results_stats(results_dic):
    ...
    """   
        ...
        n_dogs_img= sum(1 for val in results_dic.values() if val[3] == 1)
        n_notdogs_img=sum(1 for val in results_dic.values() if val[3] == 0)
        n_match=sum(1 for val in results_dic.values() if val[2] == 1)
        n_correct_dogs = sum(1 for v in results_dic.values() if v[3] == 1 and v[4] == 1)
        n_correct_notdogs=sum(1 for v in results_dic.values() if v[3] == 0 and v[4] == 0)
        n_correct_breed = sum(1 for v in results_dic.values() if v[3] == 1 and v[2] == 1)
        #percent 
        # we should always make sure that the Denominator is not zero 
